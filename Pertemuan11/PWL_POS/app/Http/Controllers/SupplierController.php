    ...
    public function update_ajax(Request $request, $id)
    {
        ...
        if ($request->ajax() || $request->wantsJson()) {
            $rules = [
                ...
                'supplier_nama' => 'required|string|max:100',
                'supplier_alamat' => 'required|string',
                'supplier_telepon' => 'required|digits_between:10,15' // Validasi telepon: hanya angka, min 10, max 15 digit
            ];
            ...
            if ($validator->fails()) {
                return response()->json([
                    'status' => false, // false jika validasi gagal
            ...
            try {
                $supplier = SupplierModel::findOrFail($id); // Pastikan data ditemukan
                ...
                return response()->json([
                    ...
                    'error' => $e->getMessage() // Tampilkan pesan error query (debugging)
                ]);
            }
        }
